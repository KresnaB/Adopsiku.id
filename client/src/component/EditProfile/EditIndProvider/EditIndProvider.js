...
import { updateIndProvider } from "../../../store/actions/userActions";
import validationEdit from "./validationEdit";

const EditIndProviderData = () => {
  ...
  useEffect(() => {
    if (Object.keys(errors).length === 0 && dataIsCorrect) {
      ...
      dispatch(updateIndProvider(profile._id, profileData, history));
    }
  }, [errors]);
    ...
    <Paper className={classes.paper} elevation={3}>
      ...
      >
        <Grid container spacing={2} className={classes.gridContainer}>
          ...
          <Grid item sm={6} className={classes.radioButton}>
...
export default EditIndProviderData;
