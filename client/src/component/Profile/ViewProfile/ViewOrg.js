...
const ViewOrg = () => {
    ...
    return (
        <Paper className={classes.paper} elevation={3}>
            <Avatar className={classes.photoProfile} variant="rounded" alt={profile.name} src={profile.imageUrl}></Avatar>
            <Typography className={classes.join} >Bergabung {moment(profile.createdAt).format('DD MMM YYYY')}</Typography>
            <Button component={Link} to={update} className={classes.auth} variant="contained" color="primary">
                Ubah Profil
            </Button>
            <Grid container>
                <Grid item sm={6} className={classes.paperProfile}>
                    <Typography className={classes.title}>Profil Organisasi Penyedia Hewan</Typography>
                    ...
                    <DataOrg label="Alamat" value={
                        (profile.address.additional? (profile.address.additional + " "):'') +
                        (profile.address.village? (profile.address.village + " " ): '') +
                        (profile.address.district? (profile.address.district + " " ): '') +
                        (profile.address.city? (profile.address.city + " "): '') +
                        (profile.address.province? ("Provinsi " + profile.address.province + " "): '')
                    }/>
                </Grid>
                <Grid item sm={6} className={classes.paperProfile}>
                    <div className={classes.titleContainer}>
                        <Typography className={classes.title} align="left">Klinik Hewan Langganan</Typography>
                    </div>
                    <DataOrg label="Nama Klinik Hewan" value={veterinarian.name}/>
                    <DataOrg label="Penanggung Jawab" value={veterinarian.personInCharge}/>
